 ...
 * Version: 4.8.2 - PRODUCTION BUILD (FIXED REDECLARATION)
 * - Resolved: Build error regarding 'formatShortDate' redeclaration
 * - Baseline: Full v4.3 logic (Employees, Routing, Backcharges)
...
const generatePDFFromHTML = (expenseList, userName, officeCode, claimNumber, submittedDate, statementImgs, reimburseCurrency, level2ApprovedBy, level2ApprovedAt) => {
  ...
  const htmlContent = `
    ...
    <html>
      ...
      <body>
        <div class="header">
          <div><h1>Staff Expense Claim Form</h1><p><strong>${office?.companyName || 'Berkeley International'}</strong></p></div>
          <div class="claim-info"><p><strong>Name:</strong> ${userName}</p><p><strong>Claim No:</strong> ${claimNumber}</p><p><strong>Date:</strong> ${new Date(submittedDate).toLocaleDateString()}</p></div>
        </div>
        <table>
          <thead><tr><th>Ref</th><th>Date</th><th>Category</th><th>Description</th><th>Original Amount</th><th>Reimbursement (${reimburseCurrency})</th></tr></thead>
          <tbody>
            ${expenseList.map(exp => `
              <tr>
                <td>${exp.ref}</td><td>${formatShortDate(exp.date)}</td><td>${EXPENSE_CATEGORIES[exp.category]?.name}</td>
                <td><strong>${exp.merchant}</strong><br/>${exp.description}${exp.hasBackcharge ? `<br/><span class="backcharge-tag">BC: ${exp.backcharges?.[0]?.development}</span>` : ''}</td>
                <td>${exp.currency} ${exp.amount}</td><td>${parseFloat(exp.reimbursementAmount || exp.amount).toFixed(2)}</td>
              </tr>`).join('')}
            <tr class="total-row"><td colspan="5" style="text-align: right">GRAND TOTAL:</td><td>${reimburseCurrency} ${expenseList.reduce((sum, e) => sum + parseFloat(e.reimbursementAmount || e.amount || 0), 0).toFixed(2)}</td></tr>
          </tbody>
        </table>
        <div class="approvals">
          <div class="approval-box">Employee Signature / Date</div>
          <div class="approval-box">Authorized by: ${level2ApprovedBy || '________________'}<br/>Date: ${level2ApprovedAt ? new Date(level2ApprovedAt).toLocaleDateString() : '________________'}</div>
        </div>
        ${statementImgs.map((img, idx) => `<div class="page-break"></div><div class="img-container"><p><strong>CC Statement Page ${idx + 1}</strong></p><img src="${img}" /></div>`).join('')}
        ${expenseList.filter(e => e.receiptPreview).map(e => `<div class="page-break"></div><div class="img-container"><p><strong>Receipt: ${e.ref} (${e.merchant})</strong></p><img src="${e.receiptPreview}" /></div>`).join('')}
      </body>
    </html>`;
...
const StatementAnnotator = ({ image, expenses, existingAnnotations = [], onSave, onCancel, pageIndex }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 flex flex-col z-[100]">
      ...
      <div className="flex-1 overflow-auto bg-slate-800 p-2"><canvas ref={canvasRef} onMouseDown={handleStart} onMouseMove={handleMove} onMouseUp={handleEnd} onMouseLeave={handleEnd} onTouchStart={handleStart} onTouchMove={handleMove} onTouchEnd={handleEnd} className={`border-2 ${selectedLabel ? 'border-orange-400' : 'border-slate-500'} rounded-lg mx-auto`} /></div>
      <div className="bg-slate-100 p-3 flex gap-3 justify-end shrink-0"><button onClick={onCancel} className="px-4 py-2 rounded-xl border-2 border-slate-300 font-semibold text-sm">Cancel</button><button onClick={handleSave} className="px-4 py-2 rounded-xl bg-green-600 text-white font-semibold text-sm">Save ✓</button></div>
    </div>
  );
};
...
const StatementUploadModal = ({ existingImages, onClose, onContinue, onEditExisting }) => {
    ...
    const handleFileSelect = (e) => { 
      ...
      reader.onload = (event) => { setLocalStatements(prev => [...prev, event.target.result]); setIsProcessing(false); };
    ...
    return (
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl max-h-[90vh] overflow-hidden flex flex-col">
          <div className="bg-amber-500 text-white p-5 shrink-0"><h2 className="text-lg font-bold">💳 Credit Card Statements</h2><p className="text-amber-100 text-sm">Tap a page thumbnail to re-annotate tags</p></div>
          ...
          <div className="p-4 border-t bg-white flex gap-3 shrink-0"><button onClick={onClose} className="flex-1 py-3 border-2 rounded-xl font-bold">Cancel</button><button onClick={() => onContinue(localStatements)} className="flex-[2] py-3 bg-green-600 text-white rounded-xl font-bold">Finish Upload →</button></div>
        </div>
      </div>
    );
};
...
export default function BerkeleyExpenseSystem() {
  ...
  const handleLogin = () => { 
    if (passwordInput === selectedEmployee.password) { 
      setCurrentUser(selectedEmployee); localStorage.setItem('berkeley_current_user', JSON.stringify(selectedEmployee));
  ...
  const handleSubmitClaim = async () => {
    ...
    if (untaggedRefs.length > 0) { alert(`⚠️ SUBMISSION BLOCKED\n\nCC items must be tagged on the statement.\nMissing: ${untaggedRefs.join(', ')}`); return; }
    setLoading(true);
    try {
      const workflow = getApprovalWorkflow(currentUser.id, currentUser.office);
      const isSelfSubmit = workflow?.selfSubmit === true;
      const claimData = { claim_number: `${currentUser.name.split(' ')[0]}-${new Date().getFullYear()}-${Date.now().toString().slice(-3)}`, user_id: currentUser.id, user_name: currentUser.name, office_code: currentUser.office, currency: getUserReimburseCurrency(currentUser), total_amount: reimbursementTotal, status: isSelfSubmit ? 'approved' : 'pending_review', approval_level: isSelfSubmit ? 2 : 1, level1_approver: workflow?.level1, level2_approver: workflow?.level2, expenses: pendingExpenses, annotated_statements: annotatedStatements, submitted_at: new Date().toISOString() };
      await supabase.from('claims').insert([claimData]);
      setExpenses([]); setAnnotatedStatements([]); setStatementAnnotations([]); await loadClaims(); alert('✅ Submitted!'); 
    } catch (err) { alert(`❌ Failed: ${err.message}`); }
    setLoading(false); setShowPreview(false);
  };
  ...
  const AddExpenseModal = ({ editExpense = null, expenses = [], onClose }) => {
    ...
    useEffect(() => {
        ...
        if (found) setDuplicateWarning(`⚠️ Duplicate amount/date detected.`); else setDuplicateWarning(null);
    ...
    return (
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-hidden shadow-2xl">
          <div className="bg-blue-600 text-white p-5 flex justify-between"><h2 className="font-bold">{editExpense ? 'Edit' : 'Add'} Expense</h2><button onClick={onClose}>✕</button></div>
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
            {step === 1 ? (
              <div className="space-y-4"><button onClick={() => setStep(2)} className="w-full p-8 border-2 border-dashed rounded-xl text-center">📸 Skip Photo / Continue</button><input type="file" onChange={(e) => { const r = new FileReader(); r.onload = (ev) => { setReceiptPreview(ev.target.result); setStep(2); }; r.readAsDataURL(e.target.files[0]); }} /></div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4"><input type="number" className="p-3 border rounded-xl" placeholder="Amount" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} /><select className="p-3 border rounded-xl" value={formData.currency} onChange={e => setFormData({...formData, currency: e.target.value})}>{CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                ...
                <div className="grid grid-cols-2 gap-4"><input type="date" className="p-3 border rounded-xl" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} /><select className="p-3 border rounded-xl" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>{Object.entries(EXPENSE_CATEGORIES).map(([k, v]) => <option key={k} value={k}>{v.icon} {v.name}</option>)}</select></div>
          ...
          <div className="p-4 border-t bg-slate-50">{step === 2 && (<div className="flex gap-3"><button onClick={() => setStep(1)} className="flex-1 py-3 border rounded-xl">Back</button><button onClick={handleSave} className="flex-[2] py-3 bg-blue-600 text-white rounded-xl font-bold">Save ✓</button></div>)}</div>
        </div>
      </div>
    );
  };
  ...
  const sortAndReassignRefs = (list) => {
    ...
    const counts = {}; return sorted.map(e => { counts[e.category] = (counts[e.category] || 0) + 1; return { ...e, ref: `${e.category}${counts[e.category]}` }; });
  };
    ...
    return (
      <div className="space-y-4">
        {reviewable.length === 0 ? <p className="text-center py-12 text-slate-400">No pending reviews</p> : reviewable.map(claim => (
          <div key={claim.id} onClick={() => setSelectedClaim(claim)} className="bg-white p-4 rounded-xl shadow-md border cursor-pointer hover:border-blue-400"><div className="font-bold text-slate-800">{claim.user_name}</div><div className="text-xs text-slate-400">{claim.claim_number}</div><div className="font-bold text-blue-600 mt-1">{formatCurrency(claim.total_amount, claim.currency)}</div></div>
        ))}
        {selectedClaim && (
           <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
             <div className="bg-white rounded-2xl w-full max-w-xl max-h-[80vh] flex flex-col shadow-2xl">
               ...
               <div className="p-4 overflow-auto flex-1 bg-slate-50"><button className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl mb-4" onClick={() => handleDownloadPDF(selectedClaim)}>📥 Download Claim PDF</button>{selectedClaim.expenses?.map((e, idx) => (<div key={idx} className="p-3 border-b text-sm flex justify-between bg-white rounded-lg mb-1 shadow-sm"><span>{e.ref} - {e.merchant}</span><b>{formatCurrency(e.reimbursementAmount || e.amount, selectedClaim.currency)}</b></div>))}</div>
               <div className="p-4 flex gap-3"><button className="flex-1 py-3 bg-red-500 text-white font-bold rounded-xl" onClick={() => handleReject(selectedClaim.id)}>Reject</button><button className="flex-1 py-3 bg-green-600 text-white font-bold rounded-xl" onClick={() => handleApprove(selectedClaim)}>Approve</button></div>
             </div>
           </div>
        )}
      </div>
  ...
  const handleApprove = async (claim) => {
    ...
    await supabase.from('claims').update({ status: level === 1 ? 'pending_level2' : 'approved', approval_level: 2, level2_approved_by: currentUser.name, level2_approved_at: new Date().toISOString() }).eq('id', claim.id);
    await loadClaims(); setSelectedClaim(null);
  };

  const handleReject = async (id) => { await supabase.from('claims').update({ status: 'changes_requested' }).eq('id', id); await loadClaims(); setSelectedClaim(null); };

  if (!currentUser) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl"><h1 className="text-3xl font-black text-center mb-8 text-slate-800">Berkeley <span className="text-blue-600">Expenses</span></h1>
          {loginStep === 'select' ? (<select className="w-full p-4 border-2 rounded-xl" onChange={(e) => { setSelectedEmployee(EMPLOYEES.find(emp => emp.id === parseInt(e.target.value))); setLoginStep('password'); }}><option value="">-- Select Your Name --</option>{EMPLOYEES.map(emp => <option key={emp.id} value={emp.id}>{emp.name}</option>)}</select>
          ) : (<div className="space-y-4"><p className="font-bold text-center">{selectedEmployee.name}</p><input type="password" placeholder="Password" className="w-full p-4 border-2 rounded-xl" value={passwordInput} onChange={e => setPasswordInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleLogin()} autoFocus /><button onClick={handleLogin} className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold">Unlock</button></div>)}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-100 pb-20">
      <header className="bg-slate-900 text-white p-4 sticky top-0 z-40 flex justify-between items-center shadow-lg"><div><div className="text-[10px] text-blue-400 font-black uppercase">Berkeley Group</div><div className="font-bold">{currentUser.name}</div></div><button className="bg-white/10 px-4 py-2 rounded-lg text-xs font-bold" onClick={() => { localStorage.clear(); window.location.reload(); }}>Logout</button></header>
      <div className="bg-white border-b sticky top-[60px] z-30 flex shadow-sm"><button onClick={() => setActiveTab('my_expenses')} className={`flex-1 py-4 text-xs font-bold uppercase border-b-4 transition-all ${activeTab === 'my_expenses' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400'}`}>My Drafts</button><button onClick={() => setActiveTab('review')} className={`flex-1 py-4 text-xs font-bold uppercase border-b-4 transition-all ${activeTab === 'review' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400'}`}>Review Claims</button></div>
      <main className="max-w-3xl mx-auto p-4">{activeTab === 'my_expenses' ? (<div className="space-y-4"><div className="bg-white p-6 rounded-2xl shadow-md flex gap-3"><button onClick={() => setShowAddExpense(true)} className="flex-1 bg-blue-600 text-white py-4 rounded-xl font-black shadow-lg">Add Receipt</button><button onClick={() => setShowPreview(true)} disabled={pendingExpenses.length === 0} className="flex-1 bg-slate-900 text-white py-4 rounded-xl font-black">Submit Claim</button></div><div className="space-y-2">{pendingExpenses.map(exp => (<div key={exp.id} className="bg-white p-4 rounded-xl border flex justify-between items-center shadow-sm"><div><div className="font-bold text-slate-800"><span className="text-blue-600 mr-1">{exp.ref}</span> {exp.merchant}</div><div className="text-[10px] text-slate-400 uppercase font-black">{exp.description}</div></div><div className="text-right font-black text-green-700">{formatCurrency(exp.reimbursementAmount || exp.amount, getUserReimburseCurrency(currentUser))}</div></div>))}</div></div>) : <ReviewClaimsTab />}</main>
      {showAddExpense && <AddExpenseModal onClose={() => setShowAddExpense(false)} expenses={expenses} />}
      {showPreview && (<div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl"><div className="p-5 bg-slate-900 text-white flex justify-between items-center"><b>Preview Claim Package</b><button onClick={() => setShowPreview(false)}>✕</button></div><div className="p-6 overflow-auto flex-1 bg-slate-50"><div className="bg-white p-5 rounded-2xl shadow-sm mb-6 flex justify-between items-center"><b>Total Amount:</b><b className="text-2xl text-blue-600">{formatCurrency(reimbursementTotal, getUserReimburseCurrency(currentUser))}</b></div>{hasForeignCurrency && (<div className={`p-5 rounded-2xl border-4 mb-6 transition-all ${annotatedStatements.length > 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-300 shadow-lg'}`}><div className="flex justify-between items-center mb-3"><div className="font-bold text-sm">{annotatedStatements.length > 0 ? '✅ Statement Attached' : '❌ Statement Required'}</div><button className="bg-amber-500 text-white px-4 py-2 rounded-xl font-bold text-xs shadow-lg" onClick={() => { setShowPreview(false); setShowStatementUpload(true); }}>{annotatedStatements.length > 0 ? 'Edit Tags' : 'Manage Statement'}</button></div></div>)}</div><div className="p-5 border-t bg-white flex gap-3"><button className="flex-1 py-4 border-2 rounded-2xl font-bold text-slate-400" onClick={() => setShowPreview(false)}>Back</button><button className={`flex-[2] py-4 rounded-2xl font-bold text-white shadow-xl ${(!hasForeignCurrency || annotatedStatements.length > 0) ? 'bg-green-600' : 'bg-slate-300 opacity-50'}`} onClick={handleSubmitClaim}>Confirm & Submit ✓</button></div></div></div>)}
      {showStatementUpload && (<StatementUploadModal existingImages={statementImages} onClose={() => setShowStatementUpload(false)} onContinue={(imgs) => { setStatementImages(imgs); setAnnotatedStatements(imgs); setShowStatementUpload(false); setShowPreview(true); }} onEditExisting={(idx, imgs) => { setStatementImages(imgs); setCurrentStatementIndex(idx); setShowStatementUpload(false); setShowStatementAnnotator(true); }} />)}
      {showStatementAnnotator && (<StatementAnnotator image={statementImages[currentStatementIndex]} expenses={pendingExpenses} existingAnnotations={statementAnnotations.filter(a => a.pageIndex === currentStatementIndex)} pageIndex={currentStatementIndex} onSave={(img, anns) => { const newAnns = [...statementAnnotations.filter(a => a.pageIndex !== currentStatementIndex), ...anns]; const newAnnotated = [...annotatedStatements]; newAnnotated[currentStatementIndex] = img; setStatementAnnotations(newAnns); setAnnotatedStatements(newAnnotated); setShowStatementAnnotator(false); setShowPreview(true); }} onCancel={() => { setShowStatementAnnotator(false); setShowPreview(true); }} />)}
    </div>
  );
}
 ...
 * Version: 2.3 - Fixed Routing + CNY Dual Receipt + PDF Subtotals
 */
...
// Standard office approval workflows
...
// Special reviewers for specific employees
...
// SENIOR STAFF ROUTING - All go to Cathy He (804) as final reviewer
...
export default function BerkeleyExpenseSystem() {
  ...
  const handleDownloadPDF = async (claim) => {
    ...
    try {
      ...
      await generatePDFFromHTML(claim.expenses || [], claim.user_name, emp?.office, claim.claim_number, claim.submitted_at, claim.annotated_statement, emp?.reimburseCurrency || claim.currency);
  ...
  const handleSubmitClaim = async () => {
    ...
    try {
      ...
      console.log('Submitting for:', currentUser.name, 'ID:', currentUser.id, 'Workflow:', workflow);
      
      if (returned) {
        await supabase.from('claims').update({ total_amount: reimbursementTotal, item_count: pendingExpenses.length, status: 'pending_review', approval_level: 1, expenses: pendingExpenses, annotated_statement: annotatedStatementImage }).eq('id', returned.id);
      } else {
        ...
        await supabase.from('claims').insert([insertData]);
      }
      setExpenses([]); setAnnotatedStatementImage(null); setStatementAnnotations([]); setStatementImageData(null);
  ...
  const handleApprove = async (claim) => {
    setLoading(true);
    const level = claim.approval_level || 1;
    if (level === 1) await supabase.from('claims').update({ status: 'pending_level2', approval_level: 2, level1_approved_by: currentUser.name, level1_approved_at: new Date().toISOString() }).eq('id', claim.id);
    else await supabase.from('claims').update({ status: 'approved', level2_approved_by: currentUser.name, level2_approved_at: new Date().toISOString() }).eq('id', claim.id);
    await loadClaims(); setSelectedClaim(null); setLoading(false);
    alert(level === 1 ? '✅ → Next reviewer' : '✅ Final Approval');
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full">
          ...
          <p className="text-center text-xs text-slate-400 mt-8">v2.3</p>
        </div>
      </div>
    );
  }
  ...
  const StatementUploadModal = () => {
    ...
    const handleFileChange = (e) => { const f = e.target.files[0]; if (f) { setFile(f); if (f.type.startsWith('image/')) { const reader = new FileReader(); reader.onloadend = () => setPreview(reader.result); reader.readAsDataURL(f); } } };
    const handleContinue = () => { setStatementImageData(preview); setShowStatementUpload(false); setShowStatementAnnotator(true); };
    return (
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl">
          ...
          <div className="p-6"><label className={`block border-3 border-dashed rounded-2xl p-6 text-center cursor-pointer ${file ? 'border-green-400 bg-green-50' : 'border-slate-300'}`}><input type="file" accept="image/*" className="hidden" onChange={handleFileChange} />{file ? (<div><span className="text-3xl">✅</span><p className="font-semibold text-green-700 mt-2">{file.name}</p>{preview && <img src={preview} alt="Preview" className="mt-3 max-h-32 mx-auto rounded" />}</div>) : (<div><span className="text-4xl">📄</span><p className="font-semibold mt-2">Upload statement image</p></div>)}</label></div>
          <div className="p-4 border-t flex gap-3"><button onClick={() => setShowStatementUpload(false)} className="flex-1 py-3 rounded-xl border-2 font-semibold">Cancel</button><button onClick={handleContinue} disabled={!file} className="flex-[2] py-3 rounded-xl bg-green-600 text-white font-semibold disabled:opacity-50">Annotate →</button></div>
        </div>
      </div>
    );
  };
        ...
        <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[95vh] overflow-hidden shadow-2xl flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <div className="max-w-3xl mx-auto border-2 border-slate-300 rounded-xl p-6">
              ...
              {hasForeignCurrency && !annotatedStatementImage && (<div className="mt-4 bg-red-50 border-2 border-red-300 rounded-xl p-4"><p className="text-red-800 font-bold">❌ Statement Required</p><button onClick={() => { setShowPreview(false); setShowStatementUpload(true); }} className="mt-2 bg-amber-500 text-white px-4 py-2 rounded-lg font-semibold text-sm">📎 Upload</button></div>)}
              {annotatedStatementImage && (<div className="mt-4 bg-green-50 border border-green-200 rounded-xl p-4"><div className="flex justify-between items-start"><p className="text-green-800 font-semibold">✅ Statement Annotated</p><button onClick={() => { setShowPreview(false); setShowStatementAnnotator(true); }} className="text-blue-600 text-sm">✏️ Edit</button></div><img src={annotatedStatementImage} alt="Statement" className="mt-2 max-h-32 rounded cursor-pointer" onClick={() => setViewImg(annotatedStatementImage)} />{untaggedExpenses.length > 0 && (<div className="mt-2 bg-amber-100 border border-amber-300 rounded-lg p-2"><p className="text-amber-800 text-sm">⚠️ Untagged: {untaggedExpenses.map(e => e.ref).join(', ')}</p></div>)}</div>)}
            </div>
          </div>
  ...
  const EditClaimModal = ({ claim, onClose }) => {
    ...
    const handleSaveEdits = async () => { setSaving(true); const newTotal = editedExpenses.reduce((sum, e) => sum + parseFloat(e.reimbursementAmount || e.amount || 0), 0); await supabase.from('claims').update({ expenses: editedExpenses, total_amount: newTotal, edited_by: currentUser.name }).eq('id', claim.id); await loadClaims(); setSaving(false); onClose(); alert('✅ Saved'); };
    return (
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50">
  ...
  const ReviewClaimsTab = () => {
    ...
    return (
      <div className="space-y-4">
        ...
        {selectedClaim && (<div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50" onClick={() => setSelectedClaim(null)}><div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-auto" onClick={e => e.stopPropagation()}><div className="p-6 border-b flex justify-between"><div><h2 className="text-xl font-bold">{selectedClaim.user_name}</h2><p className="text-sm text-slate-500">{selectedClaim.claim_number}</p></div><button onClick={() => setSelectedClaim(null)} className="text-2xl text-slate-400">×</button></div><div className="p-6"><button onClick={() => handleDownloadPDF(selectedClaim)} className="w-full bg-green-600 text-white py-3 rounded-xl font-semibold mb-4">📥 Download PDF</button>{selectedClaim.expenses?.map((exp, i) => (<div key={i} className="flex justify-between py-2 border-b"><span><span className="bg-blue-100 text-blue-700 text-xs px-2 py-0.5 rounded mr-2">{exp.ref}</span>{exp.merchant}</span><span className="font-semibold">{formatCurrency(exp.reimbursementAmount || exp.amount, selectedClaim.currency)}</span></div>))}</div><div className="p-4 border-t bg-slate-50 space-y-3"><div className="flex gap-3"><button onClick={() => setEditingClaim(selectedClaim)} className="flex-1 py-3 rounded-xl bg-purple-500 text-white font-semibold">✏️ Edit</button><button onClick={() => setShowRequestChanges(true)} className="flex-1 py-3 rounded-xl bg-amber-500 text-white font-semibold">📝 Return</button></div><div className="flex gap-3"><button onClick={() => handleReject(selectedClaim.id)} disabled={loading} className="flex-1 py-3 rounded-xl bg-red-500 text-white font-semibold disabled:opacity-50">Reject</button><button onClick={() => handleApprove(selectedClaim)} disabled={loading} className="flex-[2] py-3 rounded-xl bg-green-600 text-white font-semibold disabled:opacity-50">{(selectedClaim.approval_level || 1) === 1 ? 'Approve →' : 'Final ✓'}</button></div></div></div></div>)}
  ...
  const handleStatementAnnotationSave = (annotatedImage, annotations) => { setAnnotatedStatementImage(annotatedImage); setStatementAnnotations(annotations); setShowStatementAnnotator(false); setShowPreview(true); };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200">
      <header className="bg-gradient-to-r from-slate-900 to-slate-800 text-white px-4 py-3 shadow-lg sticky top-0 z-40">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3"><div className="text-right hidden sm:block"><div className="text-sm font-medium">{currentUser.name.split(' ').slice(0, 2).join(' ')}</div><div className="text-xs text-slate-400 capitalize">{currentUser.role}</div></div><button onClick={() => { setCurrentUser(null); setExpenses([]); setAnnotatedStatementImage(null); setStatementAnnotations([]); setStatementImageData(null); setActiveTab('my_expenses'); }} className="bg-white/10 px-3 py-2 rounded-lg text-xs font-medium">Logout</button></div>
        </div>
      </header>
      ...
      {showStatementAnnotator && statementImageData && (<StatementAnnotator image={statementImageData} expenses={pendingExpenses} existingAnnotations={statementAnnotations} onSave={handleStatementAnnotationSave} onCancel={() => { setShowStatementAnnotator(false); setShowPreview(true); }} />)}
